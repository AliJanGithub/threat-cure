...
export default function Demo() {
  ...
  const interestOptions = [
    { id: "services", label: "Services", value: "Services", description: "Our professional services offerings" },
    { id: "solutions", label: "Solutions", value: "Solutions", description: "Ready-to-use solutions" },
    { id: "development", label: "Development", value: "Development", description: "Custom development services" }
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    // Join multiple interests with comma separator
    const interestString = selectedInterests.join(",");
    const data = { fullName, email, interest: interestString };
    const query = new URLSearchParams(data).toString();
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-4 overflow-hidden relative">
      ...
      <div 
        className={`w-full max-w-2xl bg-white shadow-2xl rounded-3xl border border-orange-200 p-8 relative z-10
          transition-all duration-700 ${mounted ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95'}`}
      >
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-orange-500 to-orange-400 rounded-t-3xl"></div>
        ...
        <p className={`text-center text-gray-600 mb-6 transition-all duration-700 delay-300 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          Fill in your details and select your areas of interest
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Multi-select Interest Area */}
          <div className={`transition-all duration-500 delay-600 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <label className="block font-semibold mb-2 text-gray-700">Choose Your Areas of Interest</label>
            <p className="text-sm text-gray-500 mb-3">Select one or multiple options</p>
              ...
              <div className="mt-4">
                <div className="flex flex-wrap gap-2">
                  {selectedInterests.map((interest) => (
                    <div
                      key={interest}
                      className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-100 to-orange-50 text-orange-700 px-3 py-2 rounded-full text-sm font-medium border border-orange-200"
                    >
                      {interest}
                      <button
                        type="button"
            ...
            >
              <span className="flex items-center justify-center gap-3">
                {isSubmitting ? (
                  ...
                  <>
                    Continue to Expertise Selection
